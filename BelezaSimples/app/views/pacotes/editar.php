...
<h1>Editar Pacote</h1>
<form action="/meu_salao_app/pacotes/update" method="POST" class="bg-light p-4 rounded shadow-sm">
    <input type="hidden" name="id" value="<?php echo htmlspecialchars($pacote->id); ?>">

    <div class="mb-3">
        <label for="nome" class="form-label">Nome do Pacote:</label>
        <input type="text" class="form-control" id="nome" name="nome" value="<?php echo htmlspecialchars($pacote->nome); ?>" required>
    </div>

    <div class="mb-3">
        <label for="descricao" class="form-label">Descrição:</label>
        <textarea class="form-control" id="descricao" name="descricao" rows="3"><?php echo htmlspecialchars($pacote->descricao); ?></textarea>
    </div>

    <div class="mb-3">
        <label for="preco_total" class="form-label">Preço Total do Pacote:</label>
        <input type="number" step="0.01" class="form-control" id="preco_total" name="preco_total" value="<?php echo htmlspecialchars($pacote->preco_total); ?>" required>
    </div>
                ...
                <?php while ($servico = $todosServicos->fetch_assoc()): ?>
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="servicos[]" value="<?php echo htmlspecialchars($servico['id']); ?>" id="servico_<?php echo htmlspecialchars($servico['id']); ?>"
                            <?php echo in_array($servico['id'], $servicosAtuaisIds) ? 'checked' : ''; ?>>
    ...
    <button type="submit" class="btn btn-primary me-2">
        <i class="fas fa-sync-alt me-2"></i>Atualizar Pacote
    </button>
